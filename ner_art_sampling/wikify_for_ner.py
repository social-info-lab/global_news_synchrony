...
ALLOWED_ENTITY_TYPES = {'LAW', 'PRODUCT', 'LOC', 'LANGUAGE', 'NORP', 'WORK_OF_ART', 'GPE', 'EVENT', 'PERSON', 'ORG',
                        'FAC'}
lang_list = ['ar', 'de', 'en', 'es', 'fr', 'it', 'pl', 'ru', 'tr', 'zh']
# load traffic counts
...
for lang in lang_list:
    if os.path.exists(f"/home/xichen/mediacloud/ner_art_sampling/{key_route}/ner/{lang}/"):
        print(lang)
        run_counts = []
        match_counts = []
        concept_sizes = []
        pathlib.Path(f"/home/xichen/mediacloud/ner_art_sampling/{key_route}/wikilinked/{lang}/").mkdir(parents=True,
                                                              exist_ok=False)  # Error if there is already data there.
        for filename in tqdm(os.listdir(f"/home/xichen/mediacloud/ner_art_sampling/{key_route}/ner/{lang}")):
            # print(filename)
            run_count = 0
            match_count = 0
            if ".gz" in filename:
                fh = gzip.open(f"/home/xichen/mediacloud/ner_art_sampling/{key_route}/ner/{lang}/{filename}", mode="rt", encoding="utf-8")
                outfile = gzip.open(f"/home/xichen/mediacloud/ner_art_sampling/{key_route}/wikilinked/{lang}/{filename}", mode="wt", encoding="utf-8")
            else:
                fh = io.open(f"/home/xichen/mediacloud/ner_art_sampling/{key_route}/ner/{lang}/{filename}", mode="r", encoding="utf-8")
                outfile = gzip.open(f"/home/xichen/mediacloud/ner_art_sampling/{key_route}/wikilinked/{lang}/{filename}.gz", mode="wt", encoding="utf-8")
            for lineno, article_line in enumerate(fh):
                ...
                del article_entities["story_text"]  # Remove full text of article.
